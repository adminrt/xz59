...
$(function() {
	...
	$('#smallTab1b').on('click', function() {
		$('.index_three .small_tab_cont2').css('display', 'block');
		$('.index_three .hot_wrap2').css('display', 'none');
	});
	...
	$('.index_three .index3_menu2').on('click', function() {
		$('.index_three .small_tab_cont2').css('display', 'none');
		$('.index_three .hot_wrap2').css('display', 'block');
	});
	...
	$('.chessCard').each(function(){
		$.get("http://m.doyo.cn/location/getposcall/?",{acd:'get'},function(d){
			...
			if(d.recomdCity&&(uagent.indexOf("Baiduspider")==-1&&uagent.indexOf("Spider")==-1)){
				...
				//$(".download").remove();
				//$(".app_download").empty();
				//$(".app_download").html('<div class="appdown_no"><span>该游戏已下架</span></div>');
			}
		},"jsonp");
	});
	...
	$('.small_tab_box .small_sub_box:nth-child(12n)').after('<div class="small_sub_box"></div>'); //添加一个空白div
	var smallPre1 = 0;
	var smallPre2 = 0;
	var smallPre3 = 0;
	var smallPre4 = 0;
	var smallMenuLength1 = Math.ceil($('#smallTab1').find('li').length / 12);
	var smallMenuLength1 = Math.ceil($('#smallTab1b').find('li').length / 13);
